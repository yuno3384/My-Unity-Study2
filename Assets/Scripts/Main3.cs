...
public class Main : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // 버튼을 사용할때 주의사항
        // 프리펩으로 관리할 때 이벤트 관리 시스템가 없어지는 상황 > 반응이 없어진다
        // 이벤트 시스템은 이 클릭이벤트를 되게도 안되게도 하는
        // UI에 대해 레이캐스팅(마우스가 어느 지점을 클릭했을때 그걸 감지해주는 것)을 받아주는 것
        // 이것은 꼭 있어야한다
        // 레이캐스트 타겟, 그래픽 레이캐스터(캔버스)
        // 캔버스에 있는 그래픽 레이캐스터 : 어디를 클릭했는지 찾는 기능
        // Event System :  클릭 이벤트를 실행하는 기능
        // 이 두가지를 꼭 확인해야한다
        // 레이캐스트 : 레이저를 쏘면 어디가 맞았는지 확인

        // Canvas
        // UI는 캔버스 바깥에 있으면 아무것도 안 보인다
        //posX, posY : 게임창의 크기 > 0,0해도 문제가 안 생긴다
        // 빌드할 때는 전혀 다른 크기로 보일 수도 있다.
        // 프리 엑스펙트 > 보통은 16:9로 작업
        // 우리가 크기를 따로 만들 수도 있다

        // 흰색선은 기즈모를 끄면 안 보인다
        //렉트 트랜스폼
        // Anchor Presets  : Alt - 포지션까지 바꾼다 > 실제 위치까지 바꾼다
        // pivot : shift > 이미지의 각 꼭지점 부분 > 기준점의 위치가 바뀐다
        // 계속 눌러보는 방식으로 쓰다보면 알게 될 것이다
        // 계---속 만져보는 수 밖에 없다 > 외우지 말아라
        // 지금은 간단한 사용법을 배우는 것 나중에는 전부 코드와 같이 병행하며
        // 정말 자세하게 알아보게 될 것이다.


        //입력 기초
        // 이동을 위해 주로 하는 것
        // 클리커 게임은 만들 수 있지만, 이동을 배워야 게임을 만들 수 있다.
    ...
    void CreateNewPlayer()
    {
        ...
        // 생성함수 > 프리펩과 일치하는 복제품을 생성하는 함수
        // 생성함수는 종류가 다양하다. > 위치도 처음부터 넣을 수 있다.
