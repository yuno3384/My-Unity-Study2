...
public class NewMonoBehaviourScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            go1[i] = new GameObject();

            go1[i].name = $"Square {i+1}";

            go1[i].AddComponent<SpriteRenderer>();

            go1[i].GetComponent<SpriteRenderer>().sprite = sprite;

            go1[i].GetComponent<SpriteRenderer>().color = colorName[i];

            go1[i].transform.position = new Vector3((i-3f), 0, 0);
